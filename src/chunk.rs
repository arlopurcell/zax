...
impl Chunk {
    ...
    pub fn append(&mut self, code: ByteCode, line: u32) -> () {
        match code {
            ...
            ByteCode::DefineGlobal(arg, size) => {
                self.code.push(0x1f);
                self.code.push(arg);
                self.code.push(size);
                self.lines.push(line);
